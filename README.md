...
这是一本面向学生和工程师的雷达信号处理入门教材。本书尝试用通俗易懂的语言，从零开始讲解雷达的基本原理、信号处理方法和实际应用。本书从生活中实际问题触发，循序渐进，引导着读者一步步的了解雷达的知识。
...
1. 想要非常深入的雷达知识。（本系列是入门级教程）
2. 想要看复杂的公式。
3. 习惯填鸭式教育。

## 作者与本书的故事

保研后第一次接触雷达信号处理，被各种抽象的概念狠狠的劝退，各位前辈的教程写的全面到位，奈何本人悟性有限。我想肯定有很多人和我有一样的困惑，学不明白。
...
- `index.html` / `chapters/`：由作者本地 Markdown 稿件生成的静态网页版，每章一个页面
- `pdf/`：各章 PDF 版本
...
说明：原始 Markdown 稿件不放在本公开仓库中；这里发布的是生成后的网页、PDF 和配套 MATLAB 示例。网页插图使用在线图床链接，不在仓库中重复保存教材插图。
